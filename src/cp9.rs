...
/// C `#define LOGSUM_TBL 23000` (infernal.h:178). Table entries for indices
/// >= ~16000 round to 0, so the tail is numerically inert, but match C exactly.
pub const LOGSUM_TBL: usize = 23000;
...
use crate::constants::{
    ...
    S_ST, IL_ST, IR_ST, E_ST, EL_ST, B_ST, MP_ST, ML_ST, MR_ST,
...
const CP9_TRANS_NMATCH: usize = 4; // infernal.h:677 (CTMM,CTMI,CTMD,CTMEL)
...
fn prob2score(p: f32, null: f32) -> i32 {
    if p == 0.0 {
        ...
        let ratio = p / null;
        let v = (0.5_f64 + 1000.0_f64 * sre_log2(ratio as f64)).floor();
        // C invariant: Prob2Score never returns below -INFTY (cm.c:4222 returns
        // -INFTY for p==0, and for any valid finite p the floor stays > -INFTY).
        // A degenerate/NaN prob (e.g. the 0/0 B-state M->D transition left NaN by
        // renormalization) must map to the -INFTY "impossible" sentinel, not to a
        // saturated i32::MIN — which is dead (<= -INFTY) in ilogsum either way, but
        // i32::MIN underflows i32 when added to -INFTY in non-scan Forward. Clamp
        // to keep C's score domain [-INFTY, +] exactly. No effect on any valid score.
        if !v.is_finite() {
            -CP9_INFTY
        } else {
            (v as i32).max(-CP9_INFTY)
        }
    }
}
...
pub fn cp9_forward(
    ...
    for j in i0..=j0 {
        ...
        // C: imx[cur][0] = ESL_MAX(sc + isc[0], -INFTY) (cp9_dp.c:177). The clamp
        // pins dead cells to exactly -INFTY so a later "+ tsc" cannot underflow i32.
        // Only reachable in non-scan (alignment) Forward; scan cells stay >= -INFTY.
        imx_cur[0] = (sc + hmm.isc[0][dj]).max(ninf);
...
pub struct CP9Bands {
    pub hmm_m: i32,
    /// C cp9b->cm_M (num CM states). Set when the CM ij bands are derived.
    pub cm_m: i32,
    ...
    // ---- Truncated-alignment fields (C infernal.h:773-790, AllocCP9Bands) ----
    /// min/max cpos for which node occupancy exceeds thresh1/thresh2.
    /// sp = start position, ep = end position. Init -1 (C hmmband.c:72).
    pub sp1: i32,
    pub sp2: i32,
    pub ep1: i32,
    pub ep2: i32,
    /// CM i/j bands allowing R/L marginal alignments (C hmmband.c:75-76).
    pub rmarg_imin: i32,
    pub rmarg_imax: i32,
    pub lmarg_jmin: i32,
    pub lmarg_jmax: i32,
    /// occupancy thresholds. 'maybe used' / 'likely used' (C DEFAULT 0.01/0.98).
    pub thresh1: f32,
    pub thresh2: f32,
    /// [0..=cm_M] per-CM-state validity of the J/L/R/T marginal DP planes.
    pub jvalid: Vec<bool>,
    pub lvalid: Vec<bool>,
    pub rvalid: Vec<bool>,
    pub tvalid: Vec<bool>,
...
/// C DEFAULT_CP9BANDS_THRESH1 / THRESH2 (infernal.h:92-93).
pub const DEFAULT_CP9BANDS_THRESH1: f32 = 0.01;
pub const DEFAULT_CP9BANDS_THRESH2: f32 = 0.98;
/// C tightening bounds (infernal.h:105-108) used by the truncated iterate loop.
pub const MAX_CP9BANDS_THRESH1: f32 = 0.50;
pub const MIN_CP9BANDS_THRESH2: f32 = 0.75;
pub const DELTA_CP9BANDS_THRESH1: f32 = 0.02;
pub const DELTA_CP9BANDS_THRESH2: f32 = 0.01;

/// CP9 posterior matrix (C `pmx`), a side-output of cp9_fb2hmmbands. Only
/// mmx/dmx are read by cp9_PredictStartAndEndPositions; imx is kept for fidelity.
/// Indexed [ip][k], ip in 0..=L, k in 0..=M (same as fmx/bmx).
#[derive(Clone)]
pub struct Cp9PostMx {
    pub mmx: Vec<Vec<i32>>,
    pub imx: Vec<Vec<i32>>,
    pub dmx: Vec<Vec<i32>>,
}
...
pub fn cp9_fb2hmmbands(
    ...
    let mut b = CP9Bands {
        ...
        hdmin: Vec::new(),
        hdmax: Vec::new(),
        // truncated fields, C AllocCP9Bands defaults (hmmband.c:72-76)
        sp1: -1,
        sp2: -1,
        ep1: -1,
        ep2: -1,
        ...
        lmarg_jmin: -1,
        lmarg_jmax: -2,
        thresh1: DEFAULT_CP9BANDS_THRESH1,
        thresh2: DEFAULT_CP9BANDS_THRESH2,
        jvalid: Vec::new(),
        lvalid: Vec::new(),
        rvalid: Vec::new(),
        tvalid: Vec::new(),
    ...
    b.pn_max_d[0] = -1;
    let pmx = Cp9PostMx { mmx: pmx_m, imx: pmx_i, dmx: pmx_d };
    (b, pmx)
}

/// C: Score2Prob (cm.c:4232). Convert a scaled integer log2-odds score back to a
/// probability. `-INFTY` maps to 0. `null * sreEXP2((float)sc/INTSCALE)`.
#[inline]
fn score2prob(sc: i32, null: f32) -> f32 {
    if sc == -INFTY {
        0.0
    } else {
        null * sre_exp2((sc as f32 / 1000.0) as f64) as f32
    }
}

/// C: cp9_PredictStartAndEndPositions (hmmband.c:4801).
///
/// Given the posterior matrix `pmx` (mmx/dmx), computes per-HMM-node occupancy
/// pocc = Score2Prob(sum_i ILogsum(mmx[i][k],dmx[i][k])), and from that the
/// start/end positions sp1/sp2/ep1/ep2 (first/last node whose occupancy exceeds
/// thresh1/thresh2) and the R/L marginal CM i/j bands.
pub fn cp9_predict_start_and_end_positions(
    pmx: &Cp9PostMx,
    cp9b: &mut CP9Bands,
    i0: i32,
    j0: i32,
) {
    let l = (j0 - i0 + 1) as usize;
    let hmm_m = cp9b.hmm_m;

    // Calculate minimum start positions (C hmmband.c:4810-4835)
    let mut k = 1;
    cp9b.sp1 = -1;
    cp9b.sp2 = -1;
    while k <= hmm_m && (cp9b.sp1 == -1 || cp9b.sp2 == -1) {
        let ku = k as usize;
        if cp9b.pn_min_m[ku] == -1 && cp9b.pn_min_i[ku] == -1 && cp9b.pn_min_d[ku] == -1 {
            k += 1;
        } else {
            let mut iocc = -INFTY;
            for i in 0..=l {
                iocc = ilogsum(iocc, ilogsum(pmx.mmx[i][ku], pmx.dmx[i][ku]));
            }
            let pocc = score2prob(iocc, 1.0);
            if cp9b.sp1 == -1 && pocc > cp9b.thresh1 {
                cp9b.sp1 = k;
            }
            if cp9b.sp2 == -1 && pocc > cp9b.thresh2 {
                cp9b.sp2 = k;
            }
            k += 1;
        }
    ...
    // Calculate maximum end positions (C hmmband.c:4837-4870)
    if cp9b.sp1 == hmm_m + 1 && cp9b.sp2 == hmm_m + 1 {
        cp9b.ep1 = 0;
        cp9b.ep2 = 0;
    } else {
        cp9b.ep1 = -1;
        ...
        while k >= 1 && (cp9b.ep1 == -1 || cp9b.ep2 == -1) {
            ...
            if cp9b.pn_min_m[ku] == -1 && cp9b.pn_min_i[ku] == -1 && cp9b.pn_min_d[ku] == -1 {
                ...
                let pocc = score2prob(iocc, 1.0);
                if cp9b.ep1 == -1 && pocc > cp9b.thresh1 {
                    cp9b.ep1 = k;
                }
                if cp9b.ep2 == -1 && pocc > cp9b.thresh2 {
                    cp9b.ep2 = k;
                }
                k -= 1;
            }
    ...
    // R/L marginal i/j bands. `sp` used to derive Rmarg_i{min,max}, `ep` for
    // Lmarg_j{min,max}. Note the pn_{min,max}_d[..]-1 off-by-one for deletes
    // (HMM vs CM) at C hmmband.c:4910/4913/4925/4928.
    let mm1 = hmm_m + 1;
    let sp1 = cp9b.sp1;
    let sp2 = cp9b.sp2;
    let ep1 = cp9b.ep1;
    ...
    // Rmarg_imin (C hmmband.c:4874-4887)
    if sp1 == mm1 {
        cp9b.rmarg_imin = i0;
    } else {
        let mut r = i32::MAX;
        if sp1 != mm1 && cp9b.pn_min_m[sp1 as usize] >= 0 { r = r.min(cp9b.pn_min_m[sp1 as usize]); }
        if sp1 != mm1 && cp9b.pn_min_i[sp1 as usize] >= 0 { r = r.min(cp9b.pn_min_i[sp1 as usize]); }
        if sp1 != mm1 && cp9b.pn_min_d[sp1 as usize] >= 0 { r = r.min(cp9b.pn_min_d[sp1 as usize]); }
        if sp2 != mm1 && cp9b.pn_min_m[sp2 as usize] >= 0 { r = r.min(cp9b.pn_min_m[sp2 as usize]); }
        if sp2 != mm1 && cp9b.pn_min_i[sp2 as usize] >= 0 { r = r.min(cp9b.pn_min_i[sp2 as usize]); }
        if sp2 != mm1 && cp9b.pn_min_d[sp2 as usize] >= 0 { r = r.min(cp9b.pn_min_d[sp2 as usize]); }
        if r == i32::MAX || sp1 == mm1 || sp2 == mm1 { r = i0; }
        r = i0.max(r);
        r = (j0 + 1).min(r);
        cp9b.rmarg_imin = r;
    }

    // Rmarg_imax (C hmmband.c:4889-4902)
    if sp1 == mm1 {
        cp9b.rmarg_imax = j0;
    } else {
        let mut r = i32::MIN;
        if sp1 != mm1 && cp9b.pn_max_m[sp1 as usize] >= 0 { r = r.max(cp9b.pn_max_m[sp1 as usize]); }
        if sp1 != mm1 && cp9b.pn_max_i[sp1 as usize] >= 0 { r = r.max(cp9b.pn_max_i[sp1 as usize]); }
        if sp1 != mm1 && cp9b.pn_max_d[sp1 as usize] >= 0 { r = r.max(cp9b.pn_max_d[sp1 as usize]); }
        if sp2 != mm1 && cp9b.pn_max_m[sp2 as usize] >= 0 { r = r.max(cp9b.pn_max_m[sp2 as usize]); }
        if sp2 != mm1 && cp9b.pn_max_i[sp2 as usize] >= 0 { r = r.max(cp9b.pn_max_i[sp2 as usize]); }
        if sp2 != mm1 && cp9b.pn_max_d[sp2 as usize] >= 0 { r = r.max(cp9b.pn_max_d[sp2 as usize]); }
        if r == i32::MIN || sp1 == mm1 || sp2 == mm1 { r = j0 + 1; }
        r = i0.max(r);
        r = (j0 + 1).min(r);
        cp9b.rmarg_imax = r;
    }

    // Lmarg_jmin (C hmmband.c:4904-4917)
    if ep1 == 0 {
        cp9b.lmarg_jmin = i0 - 1;
    } else {
        let mut r = i32::MAX;
        if ep1 != 0 && cp9b.pn_min_m[ep1 as usize] >= 0 { r = r.min(cp9b.pn_min_m[ep1 as usize]); }
        if ep1 != 0 && cp9b.pn_min_i[ep1 as usize] >= 0 { r = r.min(cp9b.pn_min_i[ep1 as usize]); }
        if ep1 != 0 && cp9b.pn_min_d[ep1 as usize] >= 0 { r = r.min(cp9b.pn_min_d[ep1 as usize] - 1); }
        if ep2 != 0 && cp9b.pn_min_m[ep2 as usize] >= 0 { r = r.min(cp9b.pn_min_m[ep2 as usize]); }
        if ep2 != 0 && cp9b.pn_min_i[ep2 as usize] >= 0 { r = r.min(cp9b.pn_min_i[ep2 as usize]); }
        if ep2 != 0 && cp9b.pn_min_d[ep2 as usize] >= 0 { r = r.min(cp9b.pn_min_d[ep2 as usize] - 1); }
        if r == i32::MAX || ep1 == 0 || ep2 == 0 { r = i0 - 1; }
        r = (i0 - 1).max(r);
        r = j0.min(r);
        cp9b.lmarg_jmin = r;
    }

    // Lmarg_jmax (C hmmband.c:4919-4932)
    if ep1 == 0 {
        cp9b.lmarg_jmax = j0;
    } else {
        let mut r = i32::MIN;
        if ep1 != 0 && cp9b.pn_max_m[ep1 as usize] >= 0 { r = r.max(cp9b.pn_max_m[ep1 as usize]); }
        if ep1 != 0 && cp9b.pn_max_i[ep1 as usize] >= 0 { r = r.max(cp9b.pn_max_i[ep1 as usize]); }
        if ep1 != 0 && cp9b.pn_max_d[ep1 as usize] >= 0 { r = r.max(cp9b.pn_max_d[ep1 as usize] - 1); }
        if ep2 != 0 && cp9b.pn_max_m[ep2 as usize] >= 0 { r = r.max(cp9b.pn_max_m[ep2 as usize]); }
        if ep2 != 0 && cp9b.pn_max_i[ep2 as usize] >= 0 { r = r.max(cp9b.pn_max_i[ep2 as usize]); }
        if ep2 != 0 && cp9b.pn_max_d[ep2 as usize] >= 0 { r = r.max(cp9b.pn_max_d[ep2 as usize] - 1); }
        if r == i32::MIN || ep1 == 0 || ep2 == 0 { r = j0; }
        r = (i0 - 1).max(r);
        r = j0.min(r);
        cp9b.lmarg_jmax = r;
    }
...
/// C: cp9_MarginalCandidatesFromStartEndPositions (hmmband.c:4979).
///
/// Given sp1/sp2/ep1/ep2, decide for each CM state v whether a Joint / Left /
/// Right / Terminal marginal alignment through v is allowed, then propagate to
/// ROOT_S (v=0) and override the EL state (v=cm->M) if local ends are on.
/// `emap` must be the CM's emit map (C cm->emap). Requires cp9b.cm_m set and the
/// J/L/R/T valid arrays allocated to length cm_M+1.
pub fn cp9_marginal_candidates_from_start_end_positions(
    cm: &CM,
    emap: &EmitMap,
    cp9b: &mut CP9Bands,
    pass_idx: i32,
) {
    let cm_m = cp9b.cm_m as usize;
    // C cm_TrFillFromPassIdx: allow_L / allow_R / allow_T for this pass.
    let (allow_l, allow_r, allow_t) = crate::cm_dpsearch_trunc::fill_from_pass_idx(pass_idx);

    let (sp1, sp2, ep1, ep2) = (cp9b.sp1, cp9b.sp2, cp9b.ep1, cp9b.ep2);

    for v in 0..cm_m {
        let nd = cm.ndidx[v] as usize;
        let ndt = cm.ndtype[nd] as i32;
        // emitmap off-by-one (C hmmband.c:4994-4995)
        ...
        // C hmmband.c:5001-5015
        cp9b.jvalid[v] = lpos >= sp1 && rpos <= ep1;
        cp9b.lvalid[v] = allow_l && (lpos >= sp1 && lpos <= ep1) && (rpos > ep2);
        cp9b.rvalid[v] = allow_r && (rpos <= ep1 && rpos >= sp1) && (lpos < sp2);
        if cm.sttype[v] as i32 == B_ST {
            cp9b.tvalid[v] = allow_t && (lpos < sp2) && (rpos > ep2);
        } else {
            cp9b.tvalid[v] = false;
        }
    }

    // ROOT_S propagation (C hmmband.c:5025-5055)
    for v in 0..cm_m {
        match cm.sttype[v] as i32 {
            B_ST => {
    ...
    // EL state (v = cm->M) override (C hmmband.c:5066-5070)
    if cm.flags & CMH_LOCAL_END != 0 {
        cp9b.jvalid[cm_m] = true;
        cp9b.lvalid[cm_m] = true;
        cp9b.rvalid[cm_m] = true;
    }
}
...
/// Truncated CP9 HMMs (Rcp9, Lcp9, Tcp9) for the truncated-search passes.
///
/// C: cm_modelconfig.c:264-359 (the `CM_CONFIG_TRUNC` block). The global CP9 is
/// built (emissions/EL/transitions/renormalize) and CLONED three times BEFORE
/// cm->cp9 is put in local mode; each clone is configured with a different
/// `cp9_sw_config` swentry/swexit (always the `do_match_local_cm==FALSE` variant,
/// letting I_0/D_1/I_M stay reachable — same variant our `cp9_sw_config` already
/// is), then EL local ends are added and it is logoddsified:
///   - Lcp9 (3' truncation, L mode): swentry=0,        swexit=(M-1)/M
///   - Rcp9 (5' truncation, R mode): swentry=(M-1)/M,  swexit=0
///   - Tcp9 (5'&3' trunc,   T mode): swentry=(M-1)/M,  swexit=(M-1)/M
/// Returns (Rcp9, Lcp9, Tcp9).
pub fn cp9_build_and_configure_trunc(
    cm: &CM,
    emap: &EmitMap,
    map: &CP9Map,
    psi: &[f64],
    ...
    // Build the raw GLOBAL cp9 exactly as cp9_build_and_configure does, up to
    // (but not including) cp9_sw_config — this is cm->cp9 pre-localization, which
    // C clones from (cm_modelconfig.c:287-289).
    let mut base = cp9_build_emissions(cm, map, psi);
    cp9_init_el(cm, emap, &mut base);
    cp9_build_transitions(cm, &mut base, map, psi, tmap);
    cp9_renormalize(&mut base);

    // hmm->M == cm->clen (cp9 node count).
    let mm = base.m as f32;
    let swfull = (mm - 1.0) / mm;

    let mut lcp9 = base.clone();
    let mut rcp9 = base.clone();
    let mut tcp9 = base;

    // C cm_modelconfig.c:291-302
    cp9_sw_config(&mut lcp9, 0.0, swfull); // L: 3' trunc
    cp9_sw_config(&mut rcp9, swfull, 0.0); // R: 5' trunc
    cp9_sw_config(&mut tcp9, swfull, swfull); // T: 5'&3' trunc

    // EL-type local ends: C only applies cp9_EL_local_ends_config to the truncated
    // HMMs inside the `CM_CONFIG_LOCAL && CM_CONFIG_HMMLOCAL && CM_CONFIG_HMMEL` block
    // (cm_modelconfig.c:312-324). Truncated search passes run ONLY in GLOBAL (`-g`)
    // config, where CM_CONFIG_LOCAL is NOT set, so C does NOT add EL states to
    // Rcp9/Lcp9/Tcp9. Applying EL here diverged the CP9 Forward/Backward (and hence
    // the truncated bands) from C. Faithfully skip EL for the trunc HMMs.
    // (C cm_modelconfig.c:312-324 — EL config gated on local config.)

    // C cm_modelconfig.c:355-358
    cp9_logoddsify(&mut lcp9);
    cp9_logoddsify(&mut rcp9);
    cp9_logoddsify(&mut tcp9);

    (rcp9, lcp9, tcp9)
}

/// C: cm->{Rcp9,Lcp9,Tcp9} construction for LOCAL config (cm_modelconfig.c:287-358
/// with CM_CONFIG_LOCAL|HMMLOCAL|HMMEL all set — the cmalign default). Identical to
/// [`cp9_build_and_configure_trunc`] but additionally applies EL-type local ends to
/// each truncated HMM before logoddsify (C cm_modelconfig.c:316-322). Returns
/// (Rcp9, Lcp9, Tcp9).
pub fn cp9_build_and_configure_trunc_local(
    cm: &CM,
    emap: &EmitMap,
    map: &CP9Map,
    psi: &[f64],
    ...
    // C cm_modelconfig.c:316-322: EL local ends on each trunc HMM (local config).
    cp9_el_local_ends_config(cm, &mut lcp9);
    cp9_el_local_ends_config(cm, &mut rcp9);
    cp9_el_local_ends_config(cm, &mut tcp9);
...
/// CP9 config for sub-CM alignment (`cmalign --sub`, C CM_CONFIG_SUB block,
/// cm_modelconfig.c:331-336). Build the (global) cp9 exactly as
/// [`cp9_build_and_configure_global`], but apply `cp9_sw_config` with
/// swentry=swexit=(M-1)/M (all start/end points equiprobable, incl. 1 and M) so
/// truncated sequences can be modelled from an interior consensus column. This
/// is the `do_match_local_cm==FALSE` variant (I_0/D_1/I_M stay reachable — the
/// same variant our `cp9_sw_config` implements). NO EL states are added (CM is
/// global, CM_CONFIG_HMMLOCAL/HMMEL unset when --sub). Identical config to the
/// Tcp9 built by [`cp9_build_and_configure_trunc`], but as the single cm->cp9.
pub fn cp9_build_and_configure_sub(
    cm: &CM,
    emap: &EmitMap,
    map: &CP9Map,
    psi: &[f64],
    ...
    cp9_build_transitions(cm, &mut hmm, map, psi, tmap);
    cp9_renormalize(&mut hmm);
    // swentry = swexit = (M-1)/M (cm_modelconfig.c:332-334).
    let mm = hmm.m as f32;
    let swfull = (mm - 1.0) / mm;
    cp9_sw_config(&mut hmm, swfull, swfull);
    // No EL config for global sub cp9 (CM_CONFIG_HMMEL unset).
    cp9_logoddsify(&mut hmm);
    hmm
}

/// C: sub_build_cp9_hmm_from_mother (cp9_modelmaker.c:2596). Build the sub-CM's
/// CP9 HMM by (a) constructing the special node-0 and node-M transitions from the
/// sub_cm itself (via [`cm2hmm_special_trans_cp9`]) and renormalizing them, then
/// (b) copying the interior (1..M-1) emissions and transitions directly from the
/// mother CM's CP9, un-doing the mother's local-end mass with a `1/(1-end[mk])`
/// renormalization on the match transitions. No `cp9_sw_config`/EL config here;
/// the caller (cm_ConfigureSub) applies `cp9_sw_config(swfull,swfull)` +
/// `cp9_logoddsify` afterward. Returns (sub cp9, sub cp9map).
///
/// Contract (C 2610-2614): sub_cm must be global (no local begin/end); the
/// mother's cp9 must have LOCAL_BEGIN+LOCAL_END up and EL down.
pub fn sub_build_cp9_hmm_from_mother(
    sub_cm: &CM,
    mother_cm: &CM,
    mother_cp9: &CP9,
    spos: i32,
    ...
    // Allocate cp9map + hmm, set null model, init EL connectivity (C 2617-2625).
    let map = cp9_map_cm2hmm(sub_cm);
    let emap = create_emit_map(sub_cm);
    let mm = map.hmm_m as usize;
    let mut hmm = CP9::new(map.hmm_m);
    for i in 0..ALPHABET_SIZE {
        hmm.null[i] = sub_cm.null[i];
    }
    cp9_init_el(sub_cm, &emap, &mut hmm);

    // Transitions into node 1 and out of node M, computed from the sub_cm.
    // (C 2630-2634: cm_ExpectedStateOccupancy + cm_CreateTransitionMap +
    // cm2hmm_special_trans_cp9.)
    let psi = cm_expected_state_occupancy(sub_cm);
    let tmap = cm_create_transition_map();
    let _ = mother_cm; // mother_cm only used for the (contract-checked) cp9 below.
    cm2hmm_special_trans_cp9(sub_cm, &mut hmm, &map, &psi, &tmap);

    // Renormalize node 0 (C 2637-2647).
    let d = hmm.begin[1] + hmm.t[0][CTMI] + hmm.t[0][CTMD] + hmm.t[0][_CTMEL];
    hmm.begin[1] /= d;
    hmm.t[0][CTMI] /= d;
    hmm.t[0][CTMD] /= d;
    hmm.t[0][_CTMEL] /= d;
    fnorm(&mut hmm.t[0][CP9_TRANS_INSERT_OFFSET..CP9_TRANS_INSERT_OFFSET + 3]);
    for x in CP9_TRANS_DELETE_OFFSET..CP9_TRANS_DELETE_OFFSET + 3 {
        hmm.t[0][x] = 0.0; // esl_vec_FSet(...,0.): D_0 does not exist
    }

    // Renormalize node M (C 2649-2657).
    let d = fsum(&hmm.t[mm][0..CP9_TRANS_NMATCH]) + hmm.end[mm];
    fscale(&mut hmm.t[mm][0..CP9_TRANS_NMATCH], 1.0 / d);
    hmm.end[mm] /= d;
    ...
    // Logoddsify node 0 and node M (C 2659-2663).
    for x in 0..CP9_NTRANS {
        hmm.tsc[0][x] = prob2score(hmm.t[0][x], 1.0);
    }
    for x in 0..CP9_NTRANS {
        hmm.tsc[mm][x] = prob2score(hmm.t[mm][x], 1.0);
    }
    hmm.bsc[1] = prob2score(hmm.begin[1], 1.0);
    hmm.esc[mm] = prob2score(hmm.end[mm], 1.0);

    // Copy 1..M-1 parameters from the mother's CP9 (C 2665-2700).
    let span = (epos - spos + 1) as usize;
    for k in 0..=span {
        let mk = k + spos as usize - 1;
        if k > 0 {
            for i in 0..ALPHABET_SIZE {
                hmm.mat[k][i] = mother_cp9.mat[mk][i];
            }
            for x in 0..ALPHABET_SIZE_P {
                hmm.msc[k][x] = mother_cp9.msc[mk][x];
            }
        }
        for i in 0..ALPHABET_SIZE {
            hmm.ins[k][i] = mother_cp9.ins[mk][i];
        }
        for x in 0..ALPHABET_SIZE_P {
            hmm.isc[k][x] = mother_cp9.isc[mk][x];
        }
        if k > 1 {
            hmm.begin[k] = 0.0;
            hmm.bsc[k] = -CP9_INFTY;
        ...
        if k > 0 && k < mm {
            // Match transitions: renormalize away mother's local-end mass.
            for x in 0..CP9_TRANS_NMATCH {
                hmm.t[k][x] = mother_cp9.t[mk][x] / (1.0 - mother_cp9.end[mk]);
                hmm.tsc[k][x] = prob2score(hmm.t[k][x], 1.0);
            }
            // Insert+delete transitions copy directly (unaffected by local ends).
            for x in CP9_TRANS_INSERT_OFFSET..CP9_NTRANS {
                hmm.t[k][x] = mother_cp9.t[mk][x];
                hmm.tsc[k][x] = mother_cp9.tsc[mk][x];
            }
            hmm.end[k] = 0.0;
            hmm.esc[k] = -CP9_INFTY;
        }
    ...
    // Reordered transition scores (C 2704-2718).
    for k in 0..=mm {
        let o = &mut hmm.otsc[k];
        o[CP9O_MM] = hmm.tsc[k][CTMM];
        o[CP9O_MI] = hmm.tsc[k][CTMI];
...
pub fn cp9_hmm2ijbands(
    ...
    // C AllocCP9Bands (hmmband.c) sets cp9b->cm_M = cm->M at allocation; the CM
    // ij-band derivation here assumes it. cp9_fb2hmmbands() left cm_m=0, so set it
    // now that we are deriving the per-CM-state bands (needed by the non-truncated
    // cm_*AlignSizeNeeded* mem calc, which loops v in 0..cp9b->cm_M).
    cp9b.cm_m = cm.m;
    ...
    /* If we're allowing truncated alignments, expand bands to allow for L/R/T
     * marginal alignments (C hmmband.c:1937-1952). cp9b.{L,R}marg_{i,j}{min,max}
     * were defined in cp9_PredictStartAndEndPositions(). */
    if do_trunc {
        for v in 0..m {
            if cp9b.lvalid[v] || cp9b.tvalid[v] {
                // allow left marginal alignment by expanding j band
                cp9b.jmin[v] = if cp9b.jmin[v] == -1 { cp9b.lmarg_jmin } else { cp9b.jmin[v].min(cp9b.lmarg_jmin) };
                cp9b.jmax[v] = if cp9b.jmax[v] == -2 { cp9b.lmarg_jmax } else { cp9b.jmax[v].max(cp9b.lmarg_jmax) };
            }
            if cp9b.rvalid[v] || cp9b.tvalid[v] {
                // allow right marginal alignment by expanding i band
                cp9b.imin[v] = if cp9b.imin[v] == -1 { cp9b.rmarg_imin } else { cp9b.imin[v].min(cp9b.rmarg_imin) };
                cp9b.imax[v] = if cp9b.imax[v] == -2 { cp9b.rmarg_imax } else { cp9b.imax[v].max(cp9b.rmarg_imax) };
            }
        }
    }
    ...
    if hmm_is_localized && cm_is_fully_localized {
        if do_trunc { cp9b.jvalid[0] = true; } // C hmmband.c:2068
        if cp9b.imin[0] == -1 {
        ...
        if cm.ndtype[nd1] as i32 == BIF_ND {
            ...
            let v = cm.nodemap[nd1] as usize;
            // C hmmband.c:2162: a local begin into / local end out of v is possible,
            // so J-mode alignment through v is a candidate. (Only in the localized
            // block, so this path is exercised only by non-`-g` truncated search.)
...
/// C: cm_tr_hb_mx_SizeNeeded (cm_mx.c:1759). Megabytes needed for the truncated
/// HMM-banded DP matrix (4 J/L/R/T planes gated by cp9b {J,L,R,T}valid). Used by
/// the alignment tau-iteration size test. 64-bit sizeofs: CM_TR_HB_MX=192,
/// float**=8, int=4, float*=8, float=4.
pub fn cm_tr_hb_mx_size_needed(cm: &CM, cp9b: &CP9Bands, l: i32) -> f32 {
    let cm_m = cm.m as usize;
    let have_el = cm.flags & CMH_LOCAL_END != 0;
    let mut jncells: i64 = 0;
    let mut lncells: i64 = 0;
    let mut rncells: i64 = 0;
    let mut tncells: i64 = 0;
    let mut mb: f32 = (192 + (4 * (cm_m + 1) * 8) + (4 * (cm_m + 1) * 4)) as f32;
    for v in 0..cm_m {
        let jbw = cp9b.jmax[v] - cp9b.jmin[v];
        if cp9b.jvalid[v] {
            mb += (8 * (jbw + 1)) as f32;
            for jp in 0..=jbw {
                jncells += (cp9b.hdmax[v][jp as usize] - cp9b.hdmin[v][jp as usize] + 1) as i64;
            }
        }
        if cp9b.lvalid[v] {
            mb += (8 * (jbw + 1)) as f32;
            for jp in 0..=jbw {
                lncells += (cp9b.hdmax[v][jp as usize] - cp9b.hdmin[v][jp as usize] + 1) as i64;
            }
        }
        if cp9b.rvalid[v] {
            mb += (8 * (jbw + 1)) as f32;
            for jp in 0..=jbw {
                rncells += (cp9b.hdmax[v][jp as usize] - cp9b.hdmin[v][jp as usize] + 1) as i64;
            }
        }
        if cp9b.tvalid[v] {
            mb += (8 * (jbw + 1)) as f32;
            for jp in 0..=jbw {
                tncells += (cp9b.hdmax[v][jp as usize] - cp9b.hdmin[v][jp as usize] + 1) as i64;
            }
        }
    }
    if have_el {
        let eldeck = ((l + 2) as f64 * (l + 1) as f64 * 0.5) as i64;
        if cp9b.jvalid[cm_m] {
            mb += (8 * (l + 1)) as f32;
            jncells += eldeck;
        }
    ...
    mb += 4.0 * (jncells + lncells + rncells + tncells) as f32;
    mb *= 0.000001;
    mb
}

/// C: cp9_Seq2Bands (hmmband.c:226) TRUNCATED ALIGNMENT path (doing_search=FALSE).
/// Identical to [`cp9_seq2bands_trunc`] except doing_search is FALSE, so
/// do_fwd_scan=do_bck_scan=FALSE (Forward must start at i0, Backward end at j0),
/// the F/B run in align mode (do_align=TRUE), and cp9_HMM2ijBands gets
/// doing_search=FALSE. This is the band computation for the default (HMM-banded)
/// truncated cmalign.
#[allow(clippy::too_many_arguments)]
pub fn cp9_seq2bands_trunc_align(
    cm: &CM,
    cp9: &CP9,
    cp9map: &CP9Map,
    emap: &EmitMap,
    ...
    let doing_search = false;
    // C hmmband.c:280-283: (!doing_search) => both scans FALSE.
    let do_fwd_scan = false;
    let do_bck_scan = false;

    let (_fs, _fp, fmx, _fa) =
        cp9_forward(cp9, dsq, i0 as usize, j0 as usize, do_fwd_scan, !doing_search);
    let (_bs, _bp, bmx, _ba) =
        cp9_backward(cp9, dsq, i0 as usize, j0 as usize, do_bck_scan, !doing_search);

    let (mut cp9b, pmx) =
        cp9_fb2hmmbands(cp9, dsq, &fmx, &bmx, i0 as usize, j0 as usize, 1.0 - tau, do_fwd_scan);

    let cm_m = cm.m;
    cp9b.cm_m = cm_m;
    cp9b.thresh1 = thresh1;
    cp9b.thresh2 = thresh2;
    cp9b.jvalid = vec![true; cm_m as usize + 1];
    cp9b.lvalid = vec![true; cm_m as usize + 1];
    cp9b.rvalid = vec![true; cm_m as usize + 1];
    let mut tvalid = vec![true; cm_m as usize + 1];
    tvalid[cm_m as usize] = false;
    cp9b.tvalid = tvalid;

    cp9_predict_start_and_end_positions(&pmx, &mut cp9b, i0, j0);
    cp9_marginal_candidates_from_start_end_positions(cm, emap, &mut cp9b, pass_idx);

    cp9_hmm2ijbands(cm, cp9, &mut cp9b, cp9map, i0, j0, doing_search, true);
    ij2d_bands(cm, j0 - i0 + 1, &mut cp9b, true);

    (cp9b, pmx)
}

/// C: cp9_IterateSeq2Bands (hmmband.c:424) TRUNCATED ALIGNMENT path. Recomputes
/// truncated align bands, doubling tau (and, truncation-only, tightening
/// thresh1 up / thresh2 down) until the truncated HB DP matrix fits `size_limit`
/// Mb or all knobs hit their limits. Returns (bands, posterior mx, final_tau).
#[allow(clippy::too_many_arguments)]
pub fn cp9_iterate_seq2bands_trunc_align(
    cm: &CM,
    cp9: &CP9,
    cp9map: &CP9Map,
    emap: &EmitMap,
    ...
    pass_idx: i32,
    do_iterate: bool,
) -> (CP9Bands, Cp9PostMx, f64) {
    const TAU_MULTIPLIER: f64 = 2.0;
    let mut tau = start_tau;
    let mut thresh1 = DEFAULT_CP9BANDS_THRESH1;
    let mut thresh2 = DEFAULT_CP9BANDS_THRESH2;
    let mut tau_at_limit = false;
    let mut thresh1_at_limit = false;
    let mut thresh2_at_limit = false;
    loop {
        let (cp9b, pmx) = cp9_seq2bands_trunc_align(
            cm, cp9, cp9map, emap, dsq, i0, j0, tau, pass_idx, thresh1, thresh2,
        );
        let hbmx_mb = cm_tr_hb_mx_size_needed(cm, &cp9b, j0 - i0 + 1);
        if hbmx_mb < size_limit
            || !do_iterate
            || (tau_at_limit && thresh1_at_limit && thresh2_at_limit)
        {
            return (cp9b, pmx, tau);
        }
        if !tau_at_limit {
        ...
        if !thresh1_at_limit {
            thresh1 += DELTA_CP9BANDS_THRESH1;
            if thresh1 >= MAX_CP9BANDS_THRESH1 {
                thresh1 = MAX_CP9BANDS_THRESH1;
                thresh1_at_limit = true;
            }
        }
        if !thresh2_at_limit {
            thresh2 -= DELTA_CP9BANDS_THRESH2;
            if thresh2 <= MIN_CP9BANDS_THRESH2 {
                thresh2 = MIN_CP9BANDS_THRESH2;
                thresh2_at_limit = true;
            }
        }
    }
}
...
pub fn cp9_seq2bands(
    ...
    let (mut cp9b, _pmx) = cp9_fb2hmmbands(cp9, dsq, &fmx, &bmx, i0 as usize, j0 as usize, 1.0 - tau, do_fwd_scan);
...
/// C: cm_pli_PassEnforcesFirstRes (cm_pipeline.c:2338). TRUE if the pass forces
/// i0 (first residue) into any valid parsetree.
pub fn cm_pli_pass_enforces_first_res(pass_idx: i32) -> bool {
    use crate::cm_trunc::{PLI_PASS_5P_ONLY_FORCE, PLI_PASS_5P_AND_3P_FORCE};
    pass_idx == PLI_PASS_5P_ONLY_FORCE || pass_idx == PLI_PASS_5P_AND_3P_FORCE
}

/// C: cm_pli_PassEnforcesFinalRes (cm_pipeline.c:2367). TRUE if the pass forces
/// j0 (final residue) into any valid parsetree.
pub fn cm_pli_pass_enforces_final_res(pass_idx: i32) -> bool {
    use crate::cm_trunc::{PLI_PASS_3P_ONLY_FORCE, PLI_PASS_5P_AND_3P_FORCE};
    pass_idx == PLI_PASS_3P_ONLY_FORCE || pass_idx == PLI_PASS_5P_AND_3P_FORCE
}

/// C: cp9_Seq2Bands (hmmband.c:226) TRUNCATED path (do_trunc=TRUE, do_old_hmm2ij=
/// FALSE, use_sums=FALSE, doing_search=TRUE). Runs cp9 Forward/Backward on the
/// pass-specific truncated HMM (`cp9` = Rcp9/Lcp9/Tcp9), derives HMM bands + the
/// posterior matrix, computes the marginal start/end positions and J/L/R/T
/// validity, then the truncated CM ij and d bands. Returns (bands, posterior mx).
///
/// The caller selects `cp9` per pass_idx exactly as C cp9_Seq2Bands does
/// internally (hmmband.c:249-255): 5P->Rcp9, 3P->Lcp9, 5P&3P->Tcp9.
#[allow(clippy::too_many_arguments)]
pub fn cp9_seq2bands_trunc(
    cm: &CM,
    cp9: &CP9,
    cp9map: &CP9Map,
    emap: &EmitMap,
    ...
    let doing_search = true;
    // C hmmband.c:280-286 (doing_search, not HMMALNBANDS)
    let do_fwd_scan = !cm_pli_pass_enforces_first_res(pass_idx);
    let do_bck_scan = !cm_pli_pass_enforces_final_res(pass_idx);

    // Step 1: Forward/Backward DP.
    let (_fs, _fp, fmx, _fa) =
        cp9_forward(cp9, dsq, i0 as usize, j0 as usize, do_fwd_scan, !doing_search);
    let (_bs, _bp, bmx, _ba) =
        cp9_backward(cp9, dsq, i0 as usize, j0 as usize, do_bck_scan, !doing_search);

    // Step 2: F/B -> HMM bands + posterior matrix.
    let (mut cp9b, pmx) =
        cp9_fb2hmmbands(cp9, dsq, &fmx, &bmx, i0 as usize, j0 as usize, 1.0 - tau, do_fwd_scan);

    // C AllocCP9Bands defaults (hmmband.c:69-86): cm_M + J/L/R/T valid arrays.
    let cm_m = cm.m;
    cp9b.cm_m = cm_m;
    cp9b.thresh1 = thresh1;
    cp9b.thresh2 = thresh2;
    ...
    // Step 3: marginal start/end positions + J/L/R/T candidacy (C hmmband.c:332-335).
    cp9_predict_start_and_end_positions(&pmx, &mut cp9b, i0, j0);
    cp9_marginal_candidates_from_start_end_positions(cm, emap, &mut cp9b, pass_idx);

    // Step 4: HMM bands -> CM ij bands -> d bands (do_trunc=TRUE).
    cp9_hmm2ijbands(cm, cp9, &mut cp9b, cp9map, i0, j0, doing_search, true);
    ij2d_bands(cm, j0 - i0 + 1, &mut cp9b, true);
...
pub fn cm_logoddsify(cm: &mut CM) {
    ...
    // C CMLogoddsify (cm.c:790-864): fill per-state left/right marginal emission
    // score arrays (float lmesc/rmesc, integer ilmesc/irmesc). Foundational for the
    // truncated DP scanners (RefTrCYKScan / RefITrInsideScan).
    fill_marginal_emissions(cm);
    cm.flags |= 1 << 12; // CMH_BITS (placeholder marker)
}

/// Easel `esl_abc_FExpectScore` (esl_alphabet.c:1474): expected log-odds score of a
/// degenerate residue `x`, weighting the canonical scores by background `p`.
/// `sc[0..K]` are the canonical scores. Non-residue codes return 0.
#[inline]
fn f_expect_score(x: usize, sc: &[f32], p: &[f32]) -> f32 {
    if !crate::cm::abc_is_residue(x) {
        return 0.0;
    }
    let mut result = 0.0f32;
...
/// Easel `esl_abc_IExpectScore` (esl_alphabet.c:1457): integer version, rounded
/// half-away-from-zero. `sc[0..K]` are canonical integer scores.
#[inline]
fn i_expect_score(x: usize, sc: &[i32], p: &[f32]) -> i32 {
    if !crate::cm::abc_is_residue(x) {
        return 0;
    }
    let mut result = 0.0f32;
...
/// Easel `esl_abc_FExpectScVec` (esl_alphabet.c:1582): fill degenerate scores
/// `K+1..=Kp-3` in-place from the canonical `0..K` scores.
#[inline]
fn f_expect_scvec(sc: &mut [f32], p: &[f32]) {
    let k = ALPHABET_SIZE;
    let kp = crate::cm::ALPHABET_SIZE_P;
    for x in (k + 1)..=(kp - 3) {
        sc[x] = f_expect_score(x, sc, p);
    }
}

/// Easel `esl_abc_IExpectScVec` (esl_alphabet.c:1574).
#[inline]
fn i_expect_scvec(sc: &mut [i32], p: &[f32]) {
    let k = ALPHABET_SIZE;
    let kp = crate::cm::ALPHABET_SIZE_P;
    for x in (k + 1)..=(kp - 3) {
        sc[x] = i_expect_score(x, sc, p);
    }
}

/// C `CMLogoddsify` marginal-emission block (cm.c:790-864). Fills
/// `cm.lmesc/rmesc/ilmesc/irmesc` as full `[M][Kp]` arrays. Faithful transcription:
/// MP states get the summed left/right marginal probabilities converted to
/// log-odds and integer scores; ML/IL get lmesc=esc, rmesc=0; MR/IR get lmesc=0,
/// rmesc=esc; gap/nonresidue/missing symbols stay IMPOSSIBLE/-INFTY; degenerate
/// residues are filled via {F,I}ExpectScVec. Non-emitting states remain all-zero
/// (except gap/nonres/missing = IMPOSSIBLE/-INFTY), matching C's initial FSet(0).
pub fn fill_marginal_emissions(cm: &mut CM) {
    let m = cm.m as usize;
    let k = ALPHABET_SIZE; // 4
    let kp = crate::cm::ALPHABET_SIZE_P; // 18
    let imposs = -1e36f32;
    let ninfty = -CP9_INFTY;
    let null: [f32; ALPHABET_SIZE] = cm.null;

    cm.lmesc = vec![vec![0.0f32; kp]; m];
    cm.rmesc = vec![vec![0.0f32; kp]; m];
    cm.ilmesc = vec![vec![0i32; kp]; m];
    cm.irmesc = vec![vec![0i32; kp]; m];

    for v in 0..m {
        let stt = cm.sttype[v] as i32;
        // fill in unused marginal scores (gap K, nonresidue Kp-2, missing Kp-1)
        cm.lmesc[v][k] = imposs;
        cm.rmesc[v][k] = imposs;
        cm.lmesc[v][kp - 2] = imposs;
        cm.rmesc[v][kp - 2] = imposs;
        cm.lmesc[v][kp - 1] = imposs;
        cm.rmesc[v][kp - 1] = imposs;
        cm.ilmesc[v][k] = ninfty;
        cm.irmesc[v][k] = ninfty;
        cm.ilmesc[v][kp - 2] = ninfty;
        cm.irmesc[v][kp - 2] = ninfty;
        cm.ilmesc[v][kp - 1] = ninfty;
        cm.irmesc[v][kp - 1] = ninfty;

        if stt == crate::constants::MP_ST {
            // sum e[v][x*K+y] into left (over y) and right (over x) marginals
            let mut lm = [0.0f32; ALPHABET_SIZE];
            let mut rm = [0.0f32; ALPHABET_SIZE];
            for x in 0..k {
                for y in 0..k {
                    lm[x] += cm.e[v][x * k + y];
                    rm[y] += cm.e[v][x * k + y];
            ...
            for x in 0..k {
                cm.ilmesc[v][x] = prob2score(lm[x], null[x]);
                cm.irmesc[v][x] = prob2score(rm[x], null[x]);
                cm.lmesc[v][x] = sre_log2((lm[x] / null[x]) as f64) as f32;
                cm.rmesc[v][x] = sre_log2((rm[x] / null[x]) as f64) as f32;
            }
            f_expect_scvec(&mut cm.lmesc[v], &null);
            f_expect_scvec(&mut cm.rmesc[v], &null);
            i_expect_scvec(&mut cm.ilmesc[v], &null);
            i_expect_scvec(&mut cm.irmesc[v], &null);
        } else if stt == crate::constants::ML_ST
            || stt == crate::constants::MR_ST
            || stt == IL_ST
            || stt == IR_ST
        {
            // canonical singlet log-odds + integer scores
            let mut esc = [0.0f32; ALPHABET_SIZE];
            let mut iesc = [0i32; ALPHABET_SIZE];
            for x in 0..k {
                esc[x] = sre_log2((cm.e[v][x] / null[x]) as f64) as f32;
                iesc[x] = prob2score(cm.e[v][x], null[x]);
            }
            if stt == crate::constants::ML_ST || stt == IL_ST {
                for x in 0..k {
                    cm.lmesc[v][x] = esc[x];
                    cm.ilmesc[v][x] = iesc[x];
                    cm.rmesc[v][x] = 0.0;
                    cm.irmesc[v][x] = 0;
                }
                f_expect_scvec(&mut cm.lmesc[v], &null);
                i_expect_scvec(&mut cm.ilmesc[v], &null);
                // rmesc/irmesc degenerate positions K+1..=Kp-3 set to 0.
                for x in (k + 1)..=(kp - 3) {
                    cm.rmesc[v][x] = 0.0;
                    cm.irmesc[v][x] = 0;
                }
...
/// Contiguous max-with-constant: alpha[i0+k] = max(alpha[i0+k], c) for k<n.
/// Bit-identical to the scalar clamp `if alpha < c { alpha = c }` — `_mm256_max_ps`
/// returns (v < c) ? c : v, exactly the scalar selection (incl. tie/±0). Used for
/// the ">= IMPOSSIBLE" clamp pass (contiguous over one band row).
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
unsafe fn cyk_max_const_avx2(alpha: &mut [f32], i0: usize, n: usize, c: f32) {
    use std::arch::x86_64::*;
    let cv = _mm256_set1_ps(c);
    let mut k = 0usize;
    while k + 8 <= n {
        let vv = _mm256_loadu_ps(alpha.as_ptr().add(i0 + k));
        _mm256_storeu_ps(alpha.as_mut_ptr().add(i0 + k), _mm256_max_ps(vv, cv));
        k += 8;
    }
    while k < n {
        let av = alpha.get_unchecked_mut(i0 + k);
        if *av < c {
            *av = c;
        }
        k += 1;
...
/// Fused emission-add + IMPOSSIBLE-clamp over a contiguous band row:
/// alpha[i0+k] = max(alpha[i0+k] + em, ninf) for k<n. Used by the IR (and IL)
/// insert-state path where every cell of the row gets the same right-emission
/// score `em` added then clamped. Bit-identical to the scalar
/// `best += em; best = max(best, ninf)` — `_mm256_add_ps` is per-lane IEEE add
/// (same rounding as scalar `+`) and `_mm256_max_ps(v, ninf)` == scalar
/// `if v < ninf { v = ninf }` (same selection incl. tie/±0).
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
unsafe fn cyk_emit_clamp_avx2(alpha: &mut [f32], i0: usize, n: usize, em: f32, ninf: f32) {
    use std::arch::x86_64::*;
    let ev = _mm256_set1_ps(em);
    let nv = _mm256_set1_ps(ninf);
    let mut k = 0usize;
    while k + 8 <= n {
        let vv = _mm256_loadu_ps(alpha.as_ptr().add(i0 + k));
        let added = _mm256_add_ps(vv, ev);
        _mm256_storeu_ps(alpha.as_mut_ptr().add(i0 + k), _mm256_max_ps(added, nv));
        k += 8;
    }
    while k < n {
        let av = alpha.get_unchecked_mut(i0 + k);
...
/// Test-only runtime toggle: `INFERNOX_CYK_SCALAR=1` forces the scalar B-state /
/// clamp reference path (so the same binary can produce both the scalar reference
/// and the vectorized result for a bit-exact A/B diff). Read once.
fn cyk_force_scalar() -> bool {
    use std::sync::OnceLock;
    static F: OnceLock<bool> = OnceLock::new();
    *F.get_or_init(|| std::env::var("INFERNOX_CYK_SCALAR").is_ok())
}

/// Test-only runtime toggle: `INFERNOX_INS_SCALAR=1` forces the scalar reference
/// path inside `fast_finside_scan_hb` (independent of the CYK toggle) so the same
/// binary can emit both the scalar and the vectorized Inside result for a bit-exact
/// A/B diff. `INFERNOX_CYK_SCALAR` also disables the Inside vector path (kept for
/// backwards compatibility). Read once.
fn ins_force_scalar() -> bool {
    use std::sync::OnceLock;
    static F: OnceLock<bool> = OnceLock::new();
    *F.get_or_init(|| std::env::var("INFERNOX_INS_SCALAR").is_ok())
}

/// Test-only per-call score dump for `fast_cyk_scan_hb` (env `DUMP_F6CYK_RS`).
fn dump_f6cyk() -> bool {
    use std::sync::OnceLock;
    static F: OnceLock<bool> = OnceLock::new();
    *F.get_or_init(|| std::env::var("DUMP_F6CYK_RS").is_ok())
}

/// Test-only per-call score dump for `fast_finside_scan_hb` (env `DUMP_F7INS_RS`).
fn dump_f7ins() -> bool {
    use std::sync::OnceLock;
    static F: OnceLock<bool> = OnceLock::new();
    *F.get_or_init(|| std::env::var("DUMP_F7INS_RS").is_ok())
}
...
pub fn fast_cyk_scan_hb(
    ...
    let (sc, envi, envj, _hits) =
        fast_cyk_scan_hb_core(cm, cp9b, dsq, i0, j0, env_cutoff, false, 0.0, false);
    (sc, envi, envj)
}

/// C: FastCYKScanHB (cm_dpsearch.c:2830) with hit reporting. Same HMM-banded CYK
/// (max) recursion as [`fast_cyk_scan_hb`], but with `report_hits` also emits the
/// greedy hit list (i, j, null3-corrected score, bias) — the C `tmp_hitlist`
/// (ReportHitsGreedily, cm_mx.c:7555) used by the final Inside/CYK stage when
/// `--cyk` clears CM_SEARCH_INSIDE (cm_pipeline.c:696). `cutoff` is the reporting
/// bit-score cutoff; `do_null3` enables the act/comp/null3 correction. The
/// reporting tail is byte-identical to [`fast_finside_scan_hb`]'s (MAX + bestsc +
/// ReportHitsGreedily); only the deck-fill recursion differs (CYK max vs Inside
/// logsum), exactly as C's FastCYKScanHB differs from FastFInsideScanHB.
pub fn fast_fcyk_scan_hb(
    cm: &CM,
    cp9b: &CP9Bands,
    dsq: &[u8],
    i0: i32,
    ...
    fast_cyk_scan_hb_core(cm, cp9b, dsq, i0, j0, env_cutoff, true, cutoff, do_null3)
}

fn fast_cyk_scan_hb_core(
    cm: &CM,
    cp9b: &CP9Bands,
    dsq: &[u8],
    i0: i32,
    ...
    let m = cm.m as usize;
    let kp = crate::cm::ALPHABET_SIZE_P; // 18
    let force_scalar = cyk_force_scalar();
    #[cfg(target_arch = "x86_64")]
    let cyk_use_avx2 = is_x86_feature_detected!("avx2") && !force_scalar;
    ...
    // Build act (null3 activity): act[jp][a] = count of residue a in dsq[i0..i0+jp-1].
    // Only needed for the hit-reporting final round; env-filter callers pass report_hits=false.
    // (Identical to fast_finside_scan_hb's act; C prefills with esl_abc_DCount, canonical only.)
    let mut act: Vec<[f64; 4]> = vec![[0.0; 4]; w + 1];
    if report_hits && do_null3 {
        for jp in 1..=w {
            act[jp] = act[jp - 1];
    ...
    for v in (0..m).rev() {
        ...
        if stt == E_ST {
            ...
            #[cfg(target_arch = "x86_64")]
            let il_vec = cyk_use_avx2;
            #[cfg(not(target_arch = "x86_64"))]
            let il_vec = false;
            if il_vec {
                // IL self-transition (sdr==0) reads alpha[v][j][d-1] — the SAME j row,
                // cell d-1, whose FINAL value (esc+clamp applied) is required before
                // cell d. That is a genuine serial max-plus prefix over d and is left
                // scalar (Phase B). The *non-self* children read alpha[y][j][d-1] on
                // OTHER, already-finalized state rows and are independent across d, so
                // they are maxed in first, vectorized (Phase A, cyk_max_add_avx2).
                // Splitting non-self (all d) from self (serial) is bit-identical to the
                // scalar for d{for y} nest: ESL_MAX is associative/commutative & exact,
                // and the self-child in Phase B still reads the finalized cell d-1 (d
                // increases after Phase A completes), then esc+clamp are applied per
                // cell exactly as scalar.
                let self_off: i32 =
                    if v >= cfirst && v < cfirst + cnum { (v - cfirst) as i32 } else { -1 };
                for j in jmin[v]..=jmax[v] {
                    let jp_v = (j - jmin[v]) as usize;
                    let base_v = roff_v[jp_v];
                    let hdmin_vjp = hdmin_v[jp_v];
                    if hdmax_v[jp_v] < hdmin_vjp {
                        continue;
                    }
                    // Phase A: non-self children, vectorized over d (j_sdr == j).
                    for yoffset in 0..cnum {
                        if yoffset as i32 == self_off {
                            continue;
                        }
                        ...
                        let jp_y_sdr = (j - jmin[y]) as usize;
                        let hdmin_yj = hdmin[y][jp_y_sdr];
                        let hdmax_yj = hdmax[y][jp_y_sdr];
                        let dn = hdmin_vjp.max(hdmin_yj + sd);
                        let dx = hdmax_v[jp_v].min(hdmax_yj + sd);
                        if dx < dn {
                            continue;
                        }
                        let dp_v0 = (dn - hdmin_vjp) as usize;
                        let dp_y0 = (dn - sd - hdmin_yj) as usize;
                        let iv0 = base_v + dp_v0;
                        let iy0 = roff_flat[roff_start[y] + jp_y_sdr] + dp_y0;
                        let n = (dx - dn + 1) as usize;
                        #[cfg(target_arch = "x86_64")]
                        unsafe { cyk_max_add_avx2(&mut alpha[..], iv0, iy0, n, tsc_v[yoffset]) };
                    }
                    // Phase B: serial self-transition + left-emission + clamp, d ascending.
                    // self is always band-valid at j (j_sdr==j in [jmin_v,jmax_v]); it is
                    // legal for cell d iff (d-1) is in v's band, i.e. dp_v >= 1.
                    let tsc_self = if self_off >= 0 { tsc_v[self_off as usize] } else { 0.0 };
                    let has_self = self_off >= 0;
                    let mut d = hdmin_vjp;
                    while d <= hdmax_v[jp_v] {
                        let i = j - d + 1;
                        ...
                        if has_self && dp_v >= 1 {
                            let cand = unsafe { *alpha.get_unchecked(base_v + dp_v - 1) } + tsc_self;
                            if cand > best {
                                best = cand;
                            }
                        }
            ...
            #[cfg(target_arch = "x86_64")]
            let ir_vec = cyk_use_avx2;
            #[cfg(not(target_arch = "x86_64"))]
            let ir_vec = false;
            if ir_vec {
                // IR self-transition reads alpha[v][j-sdr=j-1][d-sd=d-1] — a *prior*
                // j row (sdr==1), fully finalized (esc+clamp applied) before row j is
                // computed since j increases. So within row j every d-cell is
                // independent of the other d-cells of the same row (no serial prefix,
                // unlike IL where sdr==0). We therefore keep the j-loop OUTERMOST (so
                // every child row — self's j-1 and all non-self states — is already
                // complete) and vectorize the d-max per child with cyk_max_add_avx2,
                // exactly like the emitting M states. Bit-identical to the scalar
                // for j{for d{for y}} nest: ESL_MAX is associative/commutative and an
                // exact selection, and esc + IMPOSSIBLE-clamp are applied per-cell
                // after all children (cyk_emit_clamp_avx2, same add+max as scalar).
                for j in jmin[v]..=jmax[v] {
                    let jp_v = (j - jmin[v]) as usize;
                    let j_sdr = j - sdr;
                    let base_v = roff_v[jp_v];
                    let hdmin_vjp = hdmin_v[jp_v];
                    let width = hdmax_v[jp_v] - hdmin_vjp;
                    if width < 0 {
                        continue;
                    }
                    for yoffset in 0..cnum {
                    ...
                    let n = (width + 1) as usize;
                    #[cfg(target_arch = "x86_64")]
                    unsafe { cyk_emit_clamp_avx2(&mut alpha[..], base_v, n, em, ninf) };
                }
            } else {
                for j in jmin[v]..=jmax[v] {
                    let jp_v = (j - jmin[v]) as usize;
            ...
            // clamp to IMPOSSIBLE — contiguous over each band row; max(alpha, ninf).
            // AVX2 `_mm256_max_ps(v, ninf)` == scalar `if v < ninf { v = ninf }`,
            // bit-identical (same selection incl. tie/±0).
            for j in jmin[v]..=jmax[v] {
                let jp_v = (j - jmin[v]) as usize;
                let width = hdmax_v[jp_v] - hdmin_v[jp_v];
                if width < 0 {
                ...
                {
                    if cyk_use_avx2 {
                        unsafe { cyk_max_const_avx2(&mut alpha[..], base_v, n, ninf) };
                    } else {
                        for k in 0..n {
                            let av = unsafe { alpha.get_unchecked_mut(base_v + k) };
                            if *av < ninf {
                                *av = ninf;
                            }
                        }
            ...
            #[cfg(target_arch = "x86_64")]
            let b_vec = cyk_use_avx2;
            #[cfg(not(target_arch = "x86_64"))]
            let b_vec = false;
            if b_vec {
                // Reordered bifurcation: swap the (d-outer, k-inner) nest to
                // (k-outer, d-inner). For a FIXED k the split constraints
                //   k >= d - hdmax_y[jp_y-k]  and  k <= d - hdmin_y[jp_y-k]
                // become a contiguous d-range  d in [k+hdmin_y[jp_y-k],
                // k+hdmax_y[jp_y-k]] ∩ v's band, and along that range:
                //   * alpha[z] cell (base_z + k-hdmin_z) is CONSTANT in d  -> scalar zsc
                //   * alpha[y] cell iy = roff_y[jp_y-k] + (d - hdmin_y[jp_y-k] - k)
                //     is CONTIGUOUS in d (stride 1)  -> the exact
                //     alpha[iv]=max(alpha[iv], alpha[iy]+zsc) pattern of
                //     cyk_max_add_avx2. Bit-identical to the scalar nest: each
                //     target cell alpha[v][d] receives the SAME candidate values
                //     (alpha[iy]+alpha[iz], same operands/order) maxed in the SAME
                //     increasing-k order, so no operand or ordering changes.
                for j in jn..=jx {
                    let jp_v = (j - jmin[v]) as usize;
                    let jp_y = j - jmin[y];
                    ...
                    let hdmin_vjp = hdmin_v[jp_v];
                    let hdmax_vjp = hdmax_v[jp_v];
                    let mut k = kn;
                    while k <= kx {
                        let jp_y_mk = (jp_y - k) as usize;
                        let hmn = hdmin_y[jp_y_mk];
                        let hmx = hdmax_y[jp_y_mk];
                        let dn2 = hdmin_vjp.max(k + hmn);
                        let dx2 = hdmax_vjp.min(k + hmx);
                        if dx2 >= dn2 {
                            let zsc = unsafe {
                                *alpha.get_unchecked(base_z + (k - hdmin_zjp) as usize)
                            };
                            let n = (dx2 - dn2 + 1) as usize;
                            let iv0 = base_v + (dn2 - hdmin_vjp) as usize;
                            let iy0 = roff_y[jp_y_mk] + (dn2 - hmn - k) as usize;
                            #[cfg(target_arch = "x86_64")]
                            unsafe {
                                cyk_max_add_avx2(&mut alpha[..], iv0, iy0, n, zsc)
                            };
                        }
                        k += 1;
                    }
    ...
    // Hit-reporting scratch (report_hits only): identical to fast_finside_scan_hb.
    ...
    for jp_v_i in 0..=jpx.max(-1) {
        ...
        // vsc_root + envelope (+ bestsc for hit reporting)
        ...
        // ReportHitsGreedily (cm_mx.c:7555) — per-j greedy hit emission. Byte-identical
        // to fast_finside_scan_hb (the reporting is MAX-based in both); gated on report_hits.
        if report_hits {
            ...
            // reset bestsc for the next j over the valid d range (match fast_finside)
            for dd in 0..=w {
                bestsc[dd] = ninf;
            }
        }
    ...
    if dump_f6cyk() {
        eprintln!(
            "[RS F6CYK] i0={} j0={} vsc_root={:.9} envi={} envj={}",
            i0, j0, vsc_root, ret_envi, ret_envj
        );
    }
...
#[inline]
pub fn flogsum(s1: f32, s2: f32) -> f32 {
    let max = s1.max(s2);
    let min = s1.min(s2);
...
pub fn fast_finside_scan_hb(
    ...
    let m = cm.m as usize;
    let kp = crate::cm::ALPHABET_SIZE_P;
    let force_scalar = cyk_force_scalar() || ins_force_scalar();
    #[cfg(target_arch = "x86_64")]
    let fi_use_avx2 = is_x86_feature_detected!("avx2") && !force_scalar;
    #[cfg(not(target_arch = "x86_64"))]
    let _ = force_scalar;
    let jmin = &cp9b.jmin;
    let jmax = &cp9b.jmax;
    ...
    for v in (0..m).rev() {
        ...
        if stt == E_ST {
            ...
            #[cfg(target_arch = "x86_64")]
            let ir_vec = fi_use_avx2;
            #[cfg(not(target_arch = "x86_64"))]
            let ir_vec = false;
            if ir_vec {
                // IR self-transition reads alpha[v][j-sdr=j-1][d-sd=d-1] — a *prior* j
                // row (sdr==1), fully finalized (esc + IMPOSSIBLE-clamp applied) before
                // row j is computed since j increases. So within row j every d-cell is
                // independent of the other d-cells of the same row (no serial FLogsum
                // prefix over d, unlike IL where sdr==0 reads the SAME row). We keep the
                // j-loop OUTERMOST (so self's j-1 row and every non-self child row are
                // already complete) and fold each child's FLogsum-add across d with
                // finside_logsum_add_avx2, exactly like the emitting M states. This is
                // BIT-IDENTICAL to the scalar for j{for d{for y}} nest: each target cell
                // alpha[v][j][d] accumulates the SAME children in the SAME increasing
                // yoffset order (finside_logsum_add_avx2 folds one child across all d
                // lanes, so the per-cell operand set + order is unchanged, and FLogsum is
                // symmetric), then esc + IMPOSSIBLE-clamp are applied per cell after all
                // children (cyk_emit_clamp_avx2 = the identical scalar add+max, applied to
                // the full [hdmin,hdmax] d-range, cells with no valid child keep their
                // init value exactly as the scalar path).
                ...
                for j in jmin[v]..=jmax[v] {
                    ...
                    for yoffset in 0..cnum {
                        ...
                        let n = (dx - dn + 1) as usize;
                        #[cfg(target_arch = "x86_64")]
                        unsafe { finside_logsum_add_avx2(&mut alpha[..], iv0, iy0, n, tsc_v[yoffset], table) };
                    }
                    let em = oesc_v[dsq[j as usize] as usize];
                    let n = (width + 1) as usize;
                    #[cfg(target_arch = "x86_64")]
            ...
            let jx = jmax[v].min(jmax[z]);
            #[cfg(target_arch = "x86_64")]
            let b_vec = fi_use_avx2;
            #[cfg(not(target_arch = "x86_64"))]
            let b_vec = false;
            if b_vec {
                // Same (k-outer, d-inner) reordering as fast_cyk_scan_hb's B-state,
                // but combining with FLogsum instead of MAX. Bit-identical: each
                // target cell alpha[v][d] accumulates flogsum over the SAME split
                // points k in the SAME increasing-k order (flogsum is symmetric and
                // the per-cell nesting order is unchanged), over the SAME operands
                // (alpha[iy]+alpha[iz], same order). alpha[z] cell is constant in d
                // (scalar zsc); alpha[y] cell is contiguous in d -> reuse the exact
                // finside_logsum_add_avx2 kernel.
                let table = &flogsum_table()[..];
                for j in jn..=jx {
                    let jp_v = (j - jmin[v]) as usize;
                    let jp_y = j - jmin[y];
                    ...
                    while k <= kx {
                        ...
                        if dx2 >= dn2 {
                            ...
                            #[cfg(target_arch = "x86_64")]
                            unsafe {
                                finside_logsum_add_avx2(&mut alpha[..], iv0, iy0, n, zsc, table)
                            };
                        }
                        k += 1;
                    }
    ...
    if dump_f7ins() {
        eprint!(
            "[RS F7INS] i0={} j0={} vsc_root={:.9} envi={} envj={} nhits={}",
            i0, j0, vsc_root, ret_envi, ret_envj, hits.len()
        );
        for h in &hits {
            eprint!(" ({},{},{:.9},{:.9})", h.0, h.1, h.2, h.3);
        }
        eprintln!();
    }
    HB_ALPHA.with(|c| *c.borrow_mut() = std::mem::take(&mut alpha));
...
/// Truncated variant of [`shift_cm_bands`] (C `cp9_ShiftCMBands(cm, i, j, do_trunc=TRUE)`,
/// hmmband.c:4633). Identical to the non-truncated shift except: (a) `max_i` uses
/// `Lp+1-ESL_MAX(sdl,sdr)` and `min_j` uses `ESL_MAX(sdl,sdr)` because in truncated
/// alignment d can be 1 for MP states (a d=1 truncated begin into an MP marginal), and
/// (b) `ij2d_bands` is called with `do_trunc=TRUE`. Used by `pli_align_hit` before a
/// truncated HB alignment (cm_pipeline.c:4328).
pub fn shift_cm_bands_trunc(cm: &CM, cp9b: &mut CP9Bands, i: i32, j: i32) {
    let ip = i - 1;
    let lp = j - i + 1;
    let m = cm.m as usize;
    for v in 0..m {
        ...
        if cp9b.imin[v] > 0 {
            // state currently reachable
            let min_i = 1;
            let max_i = lp + 1 - sdl.max(sdr);
            let min_j = sdl.max(sdr);
            let max_j = lp.max(min_j);
            cp9b.imin[v] = (cp9b.imin[v] - ip).max(min_i);
            cp9b.imax[v] = (cp9b.imax[v] - ip).min(max_i);
            cp9b.jmin[v] = (cp9b.jmin[v] - ip).max(min_j);
