...
pub fn band_truncation_negligible(density: &[f64], b: usize, z: usize) -> bool {
    // Faithful to C BandTruncationNegligible (cm_qdband.c:517-548). C has NO
    // guards on density[Z]==0 / density[b+1]==0 and NO `beta >= 1` early-out:
    // when the endpoint density has decayed to a true 0.0 (the NORMAL converged
    // case for a big-enough Z), C intends `log(0) = -inf => beta = 0 => D = 0`,
    // so `D < C*eps` is TRUE => "negligible" => Z accepted. The previous Rust
    // guards `density[z] <= 0.0 => return false` inverted this, forcing an
    // infinite Z-doubling for any model whose tail floors to exactly 0.0 (e.g.
    // 16S rRNA RF00177 at Z=4*clen), blowing the m*Z gamma allocation up to
    // ~14 GB / 78 min. C's b < Z always holds by construction; the `b+1 >= z`
    // guard is a Rust bounds-safety net (b+1==z yields NaN in C => returns 0,
    // matching our `return false`).
    if b + 1 >= z {
        return false;
    }

    // C: for (i = b+1; i <= Z; i++) C += density[i];
    let c: f64 = density[b + 1..=z].iter().sum();

    // C: logbeta = (log(density[Z]) - log(density[b+1])) / (Z - b - 1); beta = exp(logbeta);
    // density[z]==0 -> ln = -inf -> beta = 0 (intentional; see above).
    let logbeta = (density[z].ln() - density[b + 1].ln()) / (z - b - 1) as f64;
    let beta = logbeta.exp();

    // C: D = (beta / (1.-beta)) * density[Z];  (no beta>=1 guard)
    let d = (beta / (1.0 - beta)) * density[z];

    // C: if (D < C * DBL_EPSILON) return 1; else return 0;
    d < c * f64::EPSILON
}
...
pub fn calculate_query_dependent_bands(
    ...
    loop {
        if std::env::var("IX_DEBUG").is_ok() {
            eprintln!("[QDB] band_calculation_engine z={} (clen={} cap={})", z, cm.clen, cm.clen * 1000);
        }
