...
trait Sr: Copy {
    ...
    fn lt(a: Self, b: Self) -> bool; // a < b (local-begin uses MAX in BOTH semirings)
    fn is_possible(a: Self) -> bool; // NOT_IMPOSSIBLE (f32) / != -INFTY (i32) begin gate
}
impl Sr for f32 {
    #[inline]
    fn zero() -> Self {
...
/// order (cm_dpsearch.c FastIInsideScan switch(cnum), lines 1352-1475 [IL/IR
/// block] / 1498-1568 [general-emitter block]; the two blocks use identical
/// per-cnum orderings). Integer ILogsum is not associative (lookup-table
/// rounding), so the accumulation order is load-bearing for Inside byte-parity.
/// The order is harmless (order-independent) for the CYK/MAX semiring.
/// `init` = init_scAA[v][dp_y]; `terms[k]` = alpha[jp_y][y+k][dp_y] + tsc_v[k].
#[inline]
fn reduce_c_order<S: Sr>(cnum: usize, terms: &[S], init: S) -> S {
    match cnum {
        // C case 2: ILogsum(arow1+tsc1, init); ILogsum(sc, arow0+tsc0) => [1, init, 0]
        2 => {
            let mut s = S::comb(terms[1], init);
            s = S::comb(s, terms[0]);
            s
        }
        // C case 3: ILogsum(arow2+tsc2, arow1+tsc1); +init; +arow0 => [2, 1, init, 0]
        3 => {
        ...
        // C case 4: ILogsum(arow3+tsc3, arow2+tsc2); +init; +arow1; +arow0 => [3, 2, init, 1, 0]
        4 => {
        ...
        // C case 5: ILogsum(arow4+tsc4, arow3+tsc3); +init; +arow1; +arow2; +arow0
        //           => [4, 3, init, 1, 2, 0]  (note arow1 BEFORE arow2)
        5 => {
        ...
        // C case 6: ILogsum(arow5+tsc5, init); +arow4; +arow3; +arow2; +arow1; +arow0
        //           => [5, init, 4, 3, 2, 1, 0]
        6 => {
...
pub fn cm_configure_scores_global(cm: &mut CM) {
    ...
    crate::cp9::cm_logoddsify(cm);
    build_integer_scores(cm);
}

/// Build cm.itsc / cm.ioesc / cm.ibeginsc / cm.iendsc / cm.iel_selfsc, mirroring
/// C CMLogoddsify() + ICalcOptimizedEmitScores(). `pub` so the local-config scan CM
/// (already localized by `cm_configure_scores`) can gain its integer scores; a
/// localized `cm.begin[]` yields finite `ibeginsc` (local begins), unlike the global
/// path where `begin[]` stays 0 => `ibeginsc = -INFTY`.
pub fn build_integer_scores(cm: &mut CM) {
    let m = cm.m as usize;
...
struct Tables<'a, S: Sr> {
    ...
    beginsc: &'a [S],  // per-state local-begin score (S); IMPOSSIBLE/-INFTY in global
}
...
fn generic_scan<S: Sr>(
    ...
    let m = cm.m as usize;
    // CMH_LOCAL_BEGIN (cm.flags bit 10): enables the ROOT_S local-begin loop below.
    let local = (cm.flags & (1 << 10)) != 0;
    ...
    for v in (0..m).rev() {
        if cm.stid[v] as i32 != BEGL_S {
            ...
            if stt == E_ST {
                ...
                // C cm_scan_mx_Initialize{Integers,Floats}: d=0 base case uses
                // ESL_MAX (Viterbi max), NOT ILogsum, even for the Inside scan.
    ...
    for j in i0..=j0 {
        ...
        for v in (1..m).rev() {
            ...
            if stt == B_ST {
                ...
                // C cm_dpsearch.c FastIInsideScan:1333-1478: IL/IR block. Children
                // are ILogsum-folded in C's hand-unrolled switch(cnum) order, which
                // is NON-associative for the integer semiring => must match exactly.
                let y = cm.cfirst[v] as usize;
                let tsc_v = &tab.tsc[v];
                let cnum = cm.cnum[v] as usize;
                let mut i = j - dn + 1;
                ...
                // C cm_dpsearch.c FastIInsideScan:1479-1591: general (non-B,
                // non-BEGL_S, non-IL/IR) emitter block. Same hand-unrolled
                // switch(cnum) ILogsum order as the IL/IR block; NON-associative
                // for the integer Inside semiring => must match C exactly.
                let y = cm.cfirst[v] as usize;
                let tsc_v = &tab.tsc[v];
                let cnum = cm.cnum[v] as usize;
                let mut dp_y = dn - sd;
        ...
        // C cm_dpsearch.c FastCYKScan:587-621 / FastIInsideScan:1624-1651 — local
        // begins. For each y with a finite begin score: alpha[0][d] = MAX(alpha[0][d],
        // alpha[y][d] + beginsc[y]), tracking bestr[d]=y. This is a MAX in BOTH
        // semirings (the ILogsum form is commented out in C, even for Inside). BEGL_S
        // children read the alpha_begl deck at jp=j%(W+1); other states read the
        // current row (jp=cur). No-op in global config (beginsc all IMPOSSIBLE).
        if local {
            for y in 1..m {
                if !S::is_possible(tab.beginsc[y]) {
                    continue;
                }
                let dn = dn0.max(dn_v[y]);
                let dx = dx0.min((jrow as i32).min(dxcap_v[y]));
                if cm.stid[y] as i32 == BEGL_S {
                    let jpy = jp_wa[0];
                    let bi = begl_idx[y];
                    for d in dn..=dx {
                        let cand = S::addv(alpha_begl[jpy][bi][d as usize], tab.beginsc[y]);
                        if S::lt(alpha[cur][0][d as usize], cand) {
...
pub fn cyk_align_cmbounds(cm: &CM, wdsq: &[u8], hi: i32, hj: i32) -> (i32, i32) {
    ...
    let (tr, sc) = crate::cm_alidisplay::cyk_align_global(cm, subdsq, lp);
    let ad = crate::cm_alidisplay::cm_alidisplay_create(cm, &cmcons, &tr, subdsq, sc);
