...
pub fn build_glocal_profile(p7: &P7Profile, l: i32) -> GlocalProfile {
    ...
    let t0 = &p7.trans[0];
    let mut z = (t0[P7H_MD] as f64).ln();
    tsc[0 * P7P_NTRANS + P7P_BM] = (1.0 - t0[P7H_MD] as f64).ln() as f32;
    for k in 1..m {
        let tk = &p7.trans[k];
        tsc[k * P7P_NTRANS + P7P_BM] = (z + (tk[P7H_DM] as f64).ln()) as f32;
        z += (tk[P7H_DD] as f64).ln();
    }
    ...
    let mut gm = GlocalProfile { m, tsc, rsc, xsc, nj, l: 0 };
...
impl P7Gmx {
    ...
    // check; a debug_assert keeps the safety net in debug builds. Portable (no SIMD).
    #[inline]
...
/// and posterior decoding). Glocal mode: `esc = -eslINFINITY`.
pub fn p7_gforward(dsq: &[u8], l: usize, gm: &GlocalProfile, gx: &mut P7Gmx) -> f32 {
    let mm = gm.m;
    let esc = NEG_INF; // p7_profile_IsLocal(gm) ? 0 : -eslINFINITY
...
pub fn p7_domaindef_glocal(
    ...
    // C 139: process each domain in unihit mode.
    p7_reconfig_unihit(gm, save_l);
    ...
    for j in 1..=l {
        if !triggered {
            ...
            if is_multidomain_region(&btot, &etot, ri, j, rt3) {
                ...
                let clusters = glocal_region_trace_ensemble(gm, dsq, ri, j, save_l);
