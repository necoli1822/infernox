...
pub fn calculate_null3_bias(dsq: &[u8], l: i32, null_probs: &[f32; 4]) -> f32 {
    ...
    // Calculate null3 score using omega-weighted mixture
    // null3 = omega * null1 + (1-omega) * observed
    let omega = NULL3_OMEGA;

    let mut null3_score = 0.0f64;

    for i in 0..4 {
        let null1_p = null_probs[i] as f64;
        let obs_p = obs_probs[i];

        // Weighted mixture probability
        let mix_p = omega * null1_p + (1.0 - omega) * obs_p;

        if obs_p > 0.0 && mix_p > 0.0 {
            // Log-odds contribution: obs_freq * log2(null1 / mix)
            null3_score += obs_p * (null1_p / mix_p).log2();
        }
    }

    // Scale by sequence length and convert to positive bias
    let bias = -(null3_score * total_f) as f32;

    bias.max(0.0)
}

/// LogSum2: log2(2^a + 2^b) - numerically stable
/// Matches C's LogSum2() from logsum.c
pub fn log_sum2(a: f32, b: f32) -> f32 {
    let max = a.max(b);
    let min = a.min(b);

    // If min is -inf or difference > 23 bits, max dominates
...
/// C-compatible NULL3 score correction
///
/// Formula: sum(comp[a] * log2(comp[a]/null[a]) * len) + log2(omega)
/// Then: LogSum2(0, score)
pub fn score_correction_null3(
    ...
    // KL divergence weighted by composition and length
    for a in 0..4 {
        if comp[a] > 0.0 && null[a] > 0.0 {
            score += (comp[a] / null[a]).log2() * comp[a] * (len as f32);
        }
    }

    // Add omega penalty (typically -16 bits for omega=1/2^16)
    score += (omega as f32).log2();

    // Apply soft ceiling via LogSum2
    log_sum2(0.0, score)
}
...
mod tests {
    ...
    fn test_null3_bias_skewed() {
        ...
        // Bias should be positive for skewed composition
        assert!(bias > 0.0, "Skewed composition should have positive bias");
        assert!(bias > 1.0, "All A's should have significant bias, got {}", bias);
    }
